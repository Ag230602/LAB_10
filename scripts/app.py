...
if not risk_path.exists():
    st.warning(
        f"Run `PYTHONPATH=. .venv/bin/python scripts/run_pipeline.py --state {state} --use-reddit false --use-trends false` first."
    )
    st.stop()
...
with tabs[1]:
    ...
    if not temporal_edges_df.empty and {"source", "target", "timestamp", "weight"}.issubset(temporal_edges_df.columns):
        ...
        if options:
            ...
            sel = tdf[(tdf["source"] == picked["source"]) & (tdf["target"] == picked["target"]) & (tdf["relation"] == picked["relation"])].copy()
...
with tabs[2]:
    ...
    if not forecast_df.empty:
        ycols = [c for c in ["forecast", "lower_ci", "upper_ci"] if c in forecast_df.columns]
        fig = go.Figure()
        if "step" not in forecast_df.columns:
            forecast_df = forecast_df.reset_index().rename(columns={"index": "step"})
        if "forecast" in forecast_df.columns:
            fig.add_trace(go.Scatter(x=forecast_df["step"], y=forecast_df["forecast"], mode="lines+markers", name="Forecast"))
        if "lower_ci" in forecast_df.columns and "upper_ci" in forecast_df.columns:
            fig.add_trace(go.Scatter(x=forecast_df["step"], y=forecast_df["upper_ci"], mode="lines", name="Upper CI", line=dict(width=1, dash="dot")))
            fig.add_trace(go.Scatter(x=forecast_df["step"], y=forecast_df["lower_ci"], mode="lines", name="Lower CI", line=dict(width=1, dash="dot")))
